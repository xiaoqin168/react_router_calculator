...
describe('test/use-babel-eslint.test.js', () => {
  it('Default eslint parser should parse error', () => {
    ...
    assert.ok(results.messages.length === 1 && results.messages[0].fatal === true);
  });

  it('babel-eslint parser should parse well', () => {
    const configPath = './react.js';
    const filePath = path.join(__dirname, './fixtures/use-babel-eslint.jsx');
  ...
  it('babel-eslint parser should parse error for vue', () => {
    ...
    assert.ok(results.messages.length === 2);
  });
});
